...
void uart_print_nbr(uint32_t nbr)
{
	if (nbr < 10)
		uart_tx(nbr % 10 + '0');
	else
	{
		uart_print_nbr(nbr / 10);
		uart_tx(nbr % 10 + '0');
	}
}
...
int main()
{
	uart_init();
	i2c_init();
	// petit délai power-up recommandé datasheet
	_delay_ms(100);
	...
	uint16_t temp_hist[3] = {0};
	uint16_t hum_hist[3] = {0};
	uint8_t idx = 0;

	// uart_printstr("AHT20 status = 0x");
	// uart_tx("0123456789ABCDEF"[(status >> 4) & 0x0F]);
	// uart_tx("0123456789ABCDEF"[status & 0x0F]);
	// uart_printstr("\r\n");
	aht20_calibrate();
	...
	while(1)
	{
		// petit délai pour stabilisation interne (p12)
		_delay_ms(10);
		...
		humidity_x10 = (raw_humidity * 1000ULL) / 1048576;
		temperature_x10 = ((raw_temperature * 2000) / 1048576) - 500;

		temp_hist[idx] = temperature_x10;
		hum_hist[idx] = humidity_x10;

		idx++;
		if (idx == 3)
			idx = 0;

		uint32_t temp_avg = (temp_hist[0] + temp_hist[1] + temp_hist[2]) / 3;
		uint32_t hum_avg =(hum_hist[0] + hum_hist[1] + hum_hist[2]) / 3;
		
		uart_printstr("Temperature: ");
		uart_print_nbr(temp_avg / 10);
		uart_tx('.');
		uart_tx('0' + (temp_avg % 10));
		uart_tx('C');

		uart_printstr(", Humidity: ");
		uart_print_nbr(hum_avg / 10);
		uart_tx('.');
		uart_tx('0' + (hum_avg % 10));
		uart_printstr(" %\r\n");
		_delay_ms(2000);
	}
}